...
| --- | --- | --- |
| 1. Модуль крипто шифрования | Шифрует пакеты данных перед отправкой их врачу. | Работает в паре с модулем расшифрования. Даже если поток физиологических данных перенаправится на сервер злоумышленника, конфиденциальность данных сохранится. Будет реализован простой алгоритм, чтобы преобразовать данные о пациенте и состоянии экзоскелета в недоступный для чтения текст. |
| 2. Модуль проверки команд | Проверяет аутентичность входящих команд от врача к оркестратору задач. | Сверяет идентификатор отправителя и полученный при инициализации сессии. Команды без совпадения отклоняются и не передаются далее в оркестратор задач. |
| 3. Модуль расшифрования | Расшифровывает пакеты данных для врача. | Работает в паре с модулем крипто шифрования. |
| 4. Данные об экзоскелете и пациенте | Хранит информацию о состоянии систем, историю движений, уровень заряда, данные о пациенте. |  |
| 5. Оркестратор задач | Распределяет входящие команды по аварийному модулю управления и центральной системе управления. | В аварийный модуль управления передаёт команду аварийной остановки, поступающую от врача. Все остальные команды передаёт в центральную систему управления. |
| 6. Аварийный модуль управления | Перехватывает управление экзоскелетом при возникновении аварийной ситуации. Передает команды модулю аварийного открытия и модулю аварийной остановки | Получает сигнал от «критичный заряд батареи» напрямую, минуя систему мониторинга. Получает сигнал от модуля распознавания критической ситуации, системы измерения температуры внутренней части, системы контроля силы и ограничений ног и рук. При нарушении любого из условий инициирует аварийную остановку через модуль аварийной остановки и аварийное открытие кабины через модуль аварийного открытия. Работает независимо от центральной системы управления. |
| 7. Модуль аварийного открытия | Механизм аварийного открытия кабины экзоскелета. | Активируется сигналом от аварийного модуля управления. Реализован как аппаратный механизм и не может быть заблокирован программно. |
| 8. Система измерения температуры внутренней части | Измеряет температуру тела пользователя и воздуха внутри костюма при помощи датчиков. | Передаёт показания в систему терморегуляции. При опасной температуре передает сигнал в аварийный модуль управления. |
| 9. Система терморегуляции | Управляет системами нагревания и охлаждения на основе данных, полученных от системы измерения температуры внутренней части. | Активирует систему нагревания при низкой температуре и систему охлаждения при перегреве. |
| 10. Модуль верификации нейронных сигналов верхних конечностей | Проверяет аутентичность нейронных сигналов, поступающих от системы считывания нейросигналов для управления верхними конечностями. | Сравнивает идентификатор входящего сигнала с идентификатором пациента, записанным при инициализации. При несоответствии блокирует передачу сигнала в систему управления движениями руки. |
| 11. Критичный заряд батареи | Аппаратный модуль, отслеживающий пороговый уровень заряда батареи, необходимый для выполнения безопасной остановки экзоскелета. | По достижении порогового значения заряда подаёт аппаратный сигнал напрямую в аварийный модуль управления. |
| 12. Модуль верификации и ограничения тактильных сигналов | Проверяет идентификатор сигналов и ограничивает их интенсивность перед подачей на модуль отправки тактильных сигналов человеку. | Ограничивает максимальную интенсивность вибрации и стимуляции до безопасного порогового значения. Проверяет соответствие идентификатора сигнала с сигналом экзоскелета. |
| 13. Модуль распознавания критической ситуации | Анализирует данные от системы мониторинга и выявляет критические ситуации, требующие аварийной остановки. | При обнаружении непреодолимого препятствия передаёт сигнал аварийной остановки напрямую в аварийный модуль управления. |
| 14. Навигация ИНС | Физический модуль инерциальной навигационной системы, предоставляющий координаты и данные об ориентации экзоскелета в пространстве. | Передаёт данные в модуль проверки положения для комплексирования с данными GNSS. |
| 15. Навигация GNSS | Физический модуль спутниковой навигации, предоставляющий координаты экзоскелета. | Передаёт данные в Модуль проверки положения для комплексирования с данными ИНС. |
| 16. Модуль проверки положения | Комплексирует навигационные данные от навигации ИНС и навигации GNSS, проверяет нахождение экзоскелета в пределах авторизованной зоны. | При расхождении данных ИНС и GNSS сверх допустимого порога или при выходе за пределы зоны передаёт сигнал в аварийный модуль управления. |
| 17. Система контроля силы и ограничений ног | Регулирует и ограничивает усилие, развиваемое приводами нижних конечностей, в пределах биомеханически безопасных значений. | Аппаратно ограничивает максимальное усилие и скорость движения нижних конечностей. При выполнения опасного движения передаёт сигнал в аварийный модуль управления. |
| 18. Модуль верификации показаний датчиков | Проводит валидацию показаний датчиков-сенсоров путём сравнения данных нескольких датчиков между собой. | При выявлении аномального значения блокирует передачу недостоверных данных в систему мониторинга и модуль распознавания критической ситуации. |
| 19. Модуль аварийной остановки | Выполняет принудительное приведение экзоскелета в безопасную позу при получении сигнала аварийной остановки. | Получает сигнал от аварийного модуля управления. |
| 20. Критичные датчики в руках | Дают всегда верные данные о давлении прикладываемой руками. | Работают независимо от основных датчиков в руке. Передают данные напрямую в модуль контроля силы верхних конечностей. |
| 21. Критические датчики сенсоры | Набор аппаратных сенсоров (положение, ускорение, наклон, перегрузка), обеспечивающих контроль критических параметров движения. | Используются для резервной проверки корректности данных основных датчиков. |

## Качественная оценка доменов

| **Компонент** | **Оценка** |
| --- | --- |
| 1. Модуль крипто шифрования | SS |
| 2. Модуль проверки команд | SS |
| 3. Модуль расшифрования | SS |
| 4. Данные об экзоскелете и пациенте | SS |
| 5. Оркестратор задач | SS |
| 6. Аварийный модуль управления | SS |
| 7. Модуль аварийного открытия | SS |
| 8. Система измерения температуры внутренней части | SS |
| 9. Система терморегуляции | MM |
| 10. Модуль верификации нейронных сигналов верхних конечностей | SS |
| 11. Критичный заряд батареи | SS |
| 12. Модуль верификации и ограничения тактильных сигналов | SS |
| 13. Модуль распознавания критической ситуации | SS |
| 14. Навигация ИНС | MM |
| 15. Навигация GNSS | MM |
| 16. Модуль проверки положения | SS |
| 17. Система контроля силы и ограничений ног | MM |
| 18. Модуль верификации показаний датчиков | MM |
| 19. Модуль аварийной остановки | SS |
| 20. Критичные датчики в руках | MM |
| 21. Критические датчики сенсоры | MM |
