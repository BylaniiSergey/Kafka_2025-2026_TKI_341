...
| --- | --- | --- | --- |
| 1. Модуль крипто шифрования | Доверенный | Соблюдение ЦБ 3, 8 | Защищает конфиденциальность данных пациента при передаче |
| 2. Модуль проверки команд | Доверенный | Соблюдение ЦБ 1, 9 | Проверка аутентичности входящих команд от врача |
| 3. Модуль расшифрования | Доверенный | Соблюдение ЦБ 3, 8 | Работает в паре с модулем крипто шифрования |
| 4. Данные об экзоскелете и пациенте | Недоверенный |  | Хранилище данных, защита обеспечивается смежными модулями |
| 5. Оркестратор задач | Недоверенный |  | Только маршрутизирует команды, не выполняет проверок |
| 6. Аварийный модуль управления | Доверенный | Соблюдение ЦБ 2, 5 | Перехватывает управление при аварийной ситуации |
| 7. Модуль аварийного открытия | Доверенный | Соблюдение ЦБ 2, 5 | Аппаратный механизм, не блокируется программно |
| 8. Система измерения температуры внутренней части | Недоверенный |  | Источник данных от датчиков, проходит проверку далее |
| 9. Система терморегуляции | Недоверенный |  | Исполнительная подсистема |
| 10. Модуль верификации нейронных сигналов верхних конечностей | Доверенный | Соблюдение ЦБ 11 | Сверяет идентификатор сигнала с идентификатором пациента |
| 11. Критичный заряд батареи | Доверенный | Соблюдение ЦБ 2 | Аппаратный модуль, сигнал напрямую в аварийный модуль управления |
| 12. Модуль верификации и ограничения тактильных сигналов | Доверенный | Соблюдение ЦБ 5, 6 | Ограничивает интенсивность сигналов и проверяет идентификатор |
| 13. Модуль распознавания критической ситуации | Доверенный | Соблюдение ЦБ 7 | Передаёт сигнал аварийной остановки при обнаружении препятствия |
| 14. Навигация ИНС | Недоверенный |  | Аппаратный источник данных, валидируется модулем проверки положения |
| 15. Навигация GNSS | Недоверенный |  | Внешний сигнал, потенциально может быть подменён |
| 16. Модуль проверки положения | Доверенный | Соблюдение ЦБ 10 | Комплексирует ИНС и GNSS, контролирует авторизованную зону |
| 17. Система контроля силы и ограничений ног | Доверенный | Соблюдение ЦБ 4 | Аппаратно ограничивает усилие и скорость |
| 18. Модуль верификации показаний датчиков | Доверенный | Соблюдение ЦБ 2, 5 | Валидация показаний путём сравнения нескольких датчиков |
| 19. Модуль аварийной остановки | Доверенный | Соблюдение ЦБ 2, 5 | Принудительное приведение в безопасную позу |
| 20. Критичные датчики в руках | Доверенный | Соблюдение ЦБ 4, 5 | Аппаратные, работают независимо от основных датчиков |
| 21. Критические датчики сенсоры | Доверенный | Соблюдение ЦБ 4, 5 | Аппаратная резервная проверка корректности данных |

## Качественная оценка доменов

| **Компонент** | **Уровень доверия** | **Оценка** | **Кол-во входящих интерфейсов** | **Комментарий** |
| --- | --- | --- | --- | --- |
| 1. Модуль крипто шифрования | Доверенный | SS | 1 | Шифрование данных перед отправкой врачу |
| 2. Модуль проверки команд | Доверенный | SS | 1 | Сверка идентификатора отправителя с записанным при инициализации |
| 3. Модуль расшифрования | Доверенный | SS | 1 | Расшифрование пакетов данных для врача |
| 4. Данные об экзоскелете и пациенте | Недоверенный | SS | 3 | Хранилище состояния систем, истории движений и данных пациента |
| 5. Оркестратор задач | Недоверенный | SS | 1 | Распределение команд между аварийным и центральным модулями |
| 6. Аварийный модуль управления | Доверенный | SS | 5 | Получает сигналы от батареи, мониторинга, температуры, силы и проверки положения |
| 7. Модуль аварийного открытия | Доверенный | SS | 1 | Активируется сигналом от аварийного модуля управления |
| 8. Система измерения температуры внутренней части | Недоверенный | SS | 1 | Данные от температурных датчиков |
| 9. Система терморегуляции | Недоверенный | MM | 1 | Управляет нагревом/охлаждением по данным температуры |
| 10. Модуль верификации нейронных сигналов верхних конечностей | Доверенный | SS | 1 | Верификация по идентификатору пациента |
| 11. Критичный заряд батареи | Доверенный | SS | 1 | Аппаратный сигнал по достижении порогового заряда |
| 12. Модуль верификации и ограничения тактильных сигналов | Доверенный | SS | 1 | Ограничение интенсивности и проверка идентификатора |
| 13. Модуль распознавания критической ситуации | Доверенный | SS | 1 | Анализ данных от системы мониторинга |
| 14. Навигация ИНС | Недоверенный | MM | 1 | Физический модуль инерциальной навигации |
| 15. Навигация GNSS | Недоверенный | MM | 1 | Физический модуль спутниковой навигации |
| 16. Модуль проверки положения | Доверенный | SS | 2 | Комплексирование данных ИНС и GNSS |
| 17. Система контроля силы и ограничений ног | Доверенный | MM | 1 | Аппаратное ограничение усилия и скорости |
| 18. Модуль верификации показаний датчиков | Доверенный | MM | 1 | Сравнение данных нескольких датчиков между собой |
| 19. Модуль аварийной остановки | Доверенный | SS | 1 | Сигнал от аварийного модуля управления |
| 20. Критичные датчики в руках | Доверенный | MM | 0 | Аппаратные, передают данные напрямую в модуль контроля силы |
| 21. Критические датчики сенсоры | Доверенный | MM | 0 | Аппаратные сенсоры положения, ускорения, наклона, перегрузки |
