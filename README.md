...
## Негативные сценарии 1 вариант:<br>
...
<img width="1626" height="697" alt="image" src="https://github.com/user-attachments/assets/46e66ac0-588d-4d6f-aae5-4335e6878162" />
...
Исходный компонентДекомпозицияСвязьСвязьМодуль расшифрованияМодуль крипто шифрованияМодуль проверки командДанные об экзоскелете и пациентеСистема управление экзоскелетаОркестратор задачАварийный модуль управленияЦентральная система управленияСистема открытия и закрытия кабиныСистема открытия и закрытия кабиныМодуль аварийного открытияСистема контроля температуры внутренней частиСистема измерения температуры внутренней частиСистема терморегуляцииСистема считывания нейронных сигналов верхних конечностейСистема считывания нейронных сигналов верхних конечностейМодуль верификации нейронных сигналов верхних конечностейМодуль отправки тактильных сигналов человекуМодуль отправки тактильных сигналов человекуМодуль верификации и ограничения тактильных сигналовКонтроллер заряда батареиКонтроллер заряда батареиКритичный заряд батареиСистема мониторингаСистема мониторингаМодуль распознавания критической ситуацииНавигация ИНСНавигация GNSSМодуль проверки положенияСистема контроля силыСистема контроля силы и ограничений ногДатчики-сенсорыДатчики-сенсорыМодуль верификации показаний датчиковМодуль остановкиМодуль аварийной остановки
Таблица новых компонентов
КомпонентОписаниеКомментарий1. Модуль крипто шифрованияШифрует пакеты данных перед отправкой их врачу.Работает в паре с модулем расшифрования. Даже если поток физиологических данных перенаправится на сервер злоумышленника, конфиденциальность данных сохранится. Будет реализован простой алгоритм, чтобы преобразовать данные о пациенте и состоянии экзоскелета в недоступный для чтения текст.2. Модуль проверки командПроверяет аутентичность входящих команд от врача к оркестратору задач.Сверяет идентификатор отправителя и полученный при инициализации сессии. Команды без совпадения отклоняются и не передаются далее в оркестратор задач.3. Модуль расшифрованияРасшифровывает пакеты данных для врача.Работает в паре с модулем крипто шифрования.4. Данные об экзоскелете и пациентеХранит информацию о состоянии систем, историю движений, уровень заряда, данные о пациенте.5. Оркестратор задачРаспределяет входящие команды по аварийному модулю управления и центральной системе управления.В аварийный модуль управления передаёт команду аварийной остановки, поступающую от врача. Все остальные команды передаёт в центральную систему управления.6. Аварийный модуль управленияПерехватывает управление экзоскелетом при возникновении аварийной ситуации. Передает команды модулю аварийного открытия и модулю аварийной остановкиПолучает сигнал от «критичный заряд батареи» напрямую, минуя систему мониторинга. Получает сигнал от модуля распознавания критической ситуации, системы измерения температуры внутренней части, системы контроля силы и ограничений ног и рук. При нарушении любого из условий инициирует аварийную остановку через модуль аварийной остановки и аварийное открытие кабины через модуль аварийного открытия. Работает независимо от центральной системы управления.7. Модуль аварийного открытияМеханизм аварийного открытия кабины экзоскелета.Активируется сигналом от аварийного модуля управления. Реализован как аппаратный механизм и не может быть заблокирован программно.8. Система измерения температуры внутренней частиИзмеряет температуру тела пользователя и воздуха внутри костюма при помощи датчиков.Передаёт показания в систему терморегуляции. При опасной температуре передает сигнал в аварийный модуль управления.9. Система терморегуляцииУправляет системами нагревания и охлаждения на основе данных, полученных от системы измерения температуры внутренней части.Активирует систему нагревания при низкой температуре и систему охлаждения при перегреве.10. Модуль верификации нейронных сигналов верхних конечностейПроверяет аутентичность нейронных сигналов, поступающих от системы считывания нейросигналов для управления верхними конечностями.Сравнивает идентификатор входящего сигнала с идентификатором пациента, записанным при инициализации. При несоответствии блокирует передачу сигнала в систему управления движениями руки.11. Критичный заряд батареиАппаратный модуль, отслеживающий пороговый уровень заряда батареи, необходимый для выполнения безопасной остановки экзоскелета.По достижении порогового значения заряда подаёт аппаратный сигнал напрямую в аварийный модуль управления.12. Модуль верификации и ограничения тактильных сигналовПроверяет идентификатор сигналов и ограничивает их интенсивность перед подачей на модуль отправки тактильных сигналов человеку.Ограничивает максимальную интенсивность вибрации и стимуляции до безопасного порогового значения. Проверяет соответствие идентификатора сигнала с сигналом экзоскелета.13. Модуль распознавания критической ситуацииАнализирует данные от системы мониторинга и выявляет критические ситуации, требующие аварийной остановки.При обнаружении непреодолимого препятствия передаёт сигнал аварийной остановки напрямую в аварийный модуль управления.14. Навигация ИНСФизический модуль инерциальной навигационной системы, предоставляющий координаты и данные об ориентации экзоскелета в пространстве.Передаёт данные в модуль проверки положения для комплексирования с данными GNSS.15. Навигация GNSSФизический модуль спутниковой навигации, предоставляющий координаты экзоскелета.Передаёт данные в Модуль проверки положения для комплексирования с данными ИНС.16. Модуль проверки положенияКомплексирует навигационные данные от навигации ИНС и навигации GNSS, проверяет нахождение экзоскелета в пределах авторизованной зоны.При расхождении данных ИНС и GNSS сверх допустимого порога или при выходе за пределы зоны передаёт сигнал в аварийный модуль управления.17. Система контроля силы и ограничений ногРегулирует и ограничивает усилие, развиваемое приводами нижних конечностей, в пределах биомеханически безопасных значений.Аппаратно ограничивает максимальное усилие и скорость движения нижних конечностей. При выполнения опасного движения передаёт сигнал в аварийный модуль управления.18. Модуль верификации показаний датчиковПроводит валидацию показаний датчиков-сенсоров путём сравнения данных нескольких датчиков между собой.При выявлении аномального значения блокирует передачу недостоверных данных в систему мониторинга и модуль распознавания критической ситуации.19. Модуль аварийной остановкиВыполняет принудительное приведение экзоскелета в безопасную позу при получении сигнала аварийной остановки.Получает сигнал от аварийного модуля управления.20. Критичные датчики в рукахДают всегда верные данные о давлении прикладываемой руками.Работают независимо от основных датчиков в руке. Передают данные напрямую в модуль контроля силы верхних конечностей.21. Критические датчики сенсорыНабор аппаратных сенсоров (положение, ускорение, наклон, перегрузка), обеспечивающих контроль критических параметров движения.Используются для резервной проверки корректности данных основных датчиков.
Качественная оценка доменов
КомпонентОценка1. Модуль крипто шифрованияSS2. Модуль проверки командSS3. Модуль расшифрованияSS4. Данные об экзоскелете и пациентеSS5. Оркестратор задачSS6. Аварийный модуль управленияSS7. Модуль аварийного открытияSS8. Система измерения температуры внутренней частиSS9. Система терморегуляцииMM10. Модуль верификации нейронных сигналов верхних конечностейSS11. Критичный заряд батареиSS12. Модуль верификации и ограничения тактильных сигналовSS13. Модуль распознавания критической ситуацииSS14. Навигация ИНСMM15. Навигация GNSSMM16. Модуль проверки положенияSS17. Система контроля силы и ограничений ногMM18. Модуль верификации показаний датчиковMM19. Модуль аварийной остановкиSS20. Критичные датчики в рукахMM21. Критические датчики сенсорыMM


## Негативные сценарии 2 вариант:<br>
