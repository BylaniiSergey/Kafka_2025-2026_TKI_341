...
HOST = '0.0.0.0'
PORT = 8003
MODULE_NAME = os.getenv('MODULE_NAME', 'upper_arm_system')
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///upper_arm.db'
...
JOINTS_CONFIG = {
    'shoulder_flexion': {'min_angle': -45, 'max_angle': 180, 'max_speed': 60},
    'shoulder_abduction': {'min_angle': 0, 'max_angle': 180, 'max_speed': 50},
    'shoulder_rotation': {'min_angle': -90, 'max_angle': 90, 'max_speed': 45}
}
...
def calculate_movement(intent, strength):
    ...
    movements = {
        'lift_arm': {
            'shoulder_flexion': base_movement,
            'shoulder_abduction': base_movement * 0.5
        },
        'lower_arm': {
            'shoulder_flexion': -base_movement,
            'shoulder_abduction': -base_movement * 0.3
        },
        'extend_arm': {
            'shoulder_flexion': base_movement * 0.7,
            'shoulder_rotation': base_movement * 0.3
        },
        'retract_arm': {
            'shoulder_flexion': -base_movement * 0.5,
            'shoulder_abduction': base_movement * 0.2
        }
    }
...
def emergency_stop():
    ...
    return jsonify({'message': 'Upper arm emergency stop activated'})
