...
def limit_intensity(requested: float) -> tuple:
...
app = FastAPI(
    title="Tactile Verification Module", version="1.1"
)
...
def get_history(limit: int = Query(100, ge=1, le=1000)):
    ...
    try:
        logs = (
            ...
            .order_by(
                TactileVerificationLogDB.created_at.desc()
            )
            .limit(limit).all()
        )
        return [{
