...
EMERGENCY_CONTROL_URL = os.getenv('EMERGENCY_CONTROL_URL', 'http://localhost:5001')
REQUEST_TIMEOUT = 5.0
...
position_state = {
    ...
    'last_divergence': None, 'alert_active': False, 'total_alerts': 0
}
...
def compute_divergence() -> Optional[float]:
    ix, iy = position_state['ins']['x'], position_state['ins']['y']
    gx, gy = position_state['gnss']['x'], position_state['gnss']['y']
...
def send_emergency(reason: str):
    published = bus.publish(TOPIC_EMERGENCY, {
        'source': MODULE_NAME, 'reason': reason,
        'ins_position': position_state['ins'], 'gnss_position': position_state['gnss']
    })
    if published:
        return {'ok': True, 'transport': 'kafka'}
    try:
        with httpx.Client(timeout=REQUEST_TIMEOUT) as c:
            resp = c.post(f'{EMERGENCY_CONTROL_URL}/emergency', json={'source': MODULE_NAME, 'reason': reason})
            return {'ok': True, 'transport': 'http', 'http': resp.json()}
...
def evaluate_and_alert() -> dict:
    ...
    if divergence is not None and divergence > MAX_DIVERGENCE:
        alerts.append(f'INS/GNSS divergence: {divergence:.2f}')
    ins_out = not position_state['ins'].get('in_zone', True)
    if ins_out:
        reason = (f"INS: position out of zone "
                  f"(x={position_state['ins']['x']:.1f}, y={position_state['ins']['y']:.1f})")
    ...
    try:
        session.add(PositionCheckEventDB(
            ins_x=position_state['ins']['x'], ins_y=position_state['ins']['y'],
            gnss_x=position_state['gnss']['x'], gnss_y=position_state['gnss']['y'],
            divergence=divergence, ins_in_zone=position_state['ins']['in_zone'],
            gnss_in_zone=position_state['gnss']['in_zone'], trusted_source='ins',
            alert_sent=alert_sent, alert_reason='; '.join(alerts) if alerts else None
        ))
        session.commit()
    finally:
        session.close()
    return {'divergence': divergence, 'ins_in_zone': position_state['ins']['in_zone'],
            'gnss_in_zone': position_state['gnss']['in_zone'],
            'alert_sent': alert_sent, 'alerts': alerts, 'emergency_result': emergency_result}
...
@app.get('/status')
def get_status():
    return {'service': MODULE_NAME, 'ins_position': position_state['ins'],
            'gnss_position': position_state['gnss'], 'divergence': compute_divergence(),
            'max_divergence': MAX_DIVERGENCE, 'zone_limit': ZONE_LIMIT,
            'alert_active': position_state['alert_active'],
            'total_alerts': position_state['total_alerts'], 'trusted_source': 'ins'}
...
def ins_update(body: PositionUpdateRequest):
    ...
    return {'ok': True, 'source': 'ins', 'position': {'x': body.x, 'y': body.y}, 'evaluation': result}
...
def gnss_update(body: PositionUpdateRequest):
    ...
    result = evaluate_and_alert()
    return {'ok': True, 'source': 'gnss', 'position': {'x': body.x, 'y': body.y}, 'evaluation': result}
...
def get_history(limit: int = Query(100, ge=1, le=1000)):
    session = SessionLocal()
    try:
        events = session.query(PositionCheckEventDB).order_by(PositionCheckEventDB.created_at.desc()).limit(limit).all()
        return [{'id': e.id, 'ins_x': e.ins_x, 'ins_y': e.ins_y, 'gnss_x': e.gnss_x, 'gnss_y': e.gnss_y,
                 'divergence': e.divergence, 'alert_sent': e.alert_sent, 'alert_reason': e.alert_reason,
