...
def analyze_phantom_signals(
    ...
    if target_arm == TargetArm.RIGHT:
        ...
        sf = max(signals.get('stump_right_front', 0),
                 signals.get('stump_left_front', 0))
        sb = max(signals.get('stump_right_back', 0),
                 signals.get('stump_left_back', 0))
        tr = max(signals.get('shoulder_right_trapezius', 0),
                 signals.get('shoulder_left_trapezius', 0))
        dl = max(signals.get('shoulder_right_deltoid', 0),
...
def analyze_body_signals(
    ...
    if target_arm == TargetArm.RIGHT:
        ...
        chest = max(signals.get('chest_right', 0),
                    signals.get('chest_left', 0))

    nc = normalize_signal(chest)
    strength = max(0.1, min(1.0, nc))
    speed_modifier = 1.5 if nc > 0.7 else (1.0 if nc > 0.4 else 0.6)
...
app = FastAPI(title="Neural Signal System", version="3.0")
...
def analyze_signals(body: SignalsInput = SignalsInput()):
    ...
    try:
        session.add(SignalReadingDB(
            ...
            shoulder_right_trapezius=signals.get(
                'shoulder_right_trapezius', 0
            ),
            shoulder_right_deltoid=signals.get(
                'shoulder_right_deltoid', 0
            ),
            shoulder_left_trapezius=signals.get(
    ...
    # Пересылаем в neural_verify_upper → arm_force_limits → arm_movement
    verify_result = None
    if body.forward_to_verify and result['can_execute']:
        try:
            with httpx.Client(timeout=REQUEST_TIMEOUT) as c:
                ...
                forwarded = 'yes'
                logger.info(f"Forwarded to verify: {verify_result}")
...
def get_readings(limit: int = Query(100, ge=1, le=1000)):
    ...
    try:
        ...
        return [{'id': r.id, 'arm': r.detected_arm,
                 'intent': r.detected_intent, 'strength': r.strength,
                 'forwarded': r.forwarded} for r in readings]
    finally:
        session.close()
