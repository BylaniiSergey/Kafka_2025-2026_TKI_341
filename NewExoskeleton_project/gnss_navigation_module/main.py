# gnss_navigation_module/main.py
import os
import logging
...
def receive_movement_event(body: MovementEventRequest):
    ...
    if dx == 0.0 and dy == 0.0:
        return {
            'ok': True,
            'message': (
                f'Intent {body.intent} does not affect position'
            ),
            'position': {
                'x': gnss_state['x'],
                'y': gnss_state['y']
    ...
    logger.info(
        f"GNSS position: x={gnss_state['x']:.1f}, "
        f"y={gnss_state['y']:.1f}, in_zone={in_zone}"
    )
...
def reset_position():
    ...
    gnss_state['step_count'] = {
        k: 0 for k in gnss_state['step_count']
    }
    save_position('reset')
    return {
        'ok': True,
...
def get_history(limit: int = Query(100, ge=1, le=1000)):
    ...
    try:
        positions = (
            session.query(GNSSPositionDB)
            .order_by(GNSSPositionDB.created_at.desc())
            .limit(limit).all()
        )
        return [{
            'id': p.id,
