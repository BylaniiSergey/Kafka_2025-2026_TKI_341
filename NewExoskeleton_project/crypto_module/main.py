...
MASTER_KEY = os.getenv(
    'CRYPTO_MASTER_KEY',
    'test-key-do-not-use-in-prod-32chars!!'
).encode()
...
DATABASE_URL = os.getenv(
    'DATABASE_URL', 'sqlite:///crypto_module.db'
)
...
def sign(data: str, key: bytes) -> str:
    return hmac.new(
        key, data.encode(), hashlib.sha256
    ).hexdigest()


def save_log(
    op: str, src: str, tgt: str, success: bool
):
    session = SessionLocal()
    try:
        session.add(CryptoLogDB(
            operation=op,
            source_module=src,
            target_module=tgt,
            success=success
...
app = FastAPI(title="Crypto Module", version="2.1")
...
def encrypt(body: EncryptRequest):
    if not fernet:
        raise HTTPException(
            503, "Crypto subsystem unavailable"
        )
    try:
...
def decrypt(body: DecryptRequest):
    ...
    if not hmac.compare_digest(
        sign(body.ciphertext, MASTER_KEY),
        body.signature
    ):
    ...
    try:
        ...
        save_log('decrypt', body.source, body.target, False)
        raise HTTPException(
            400, f"Decryption failed: {str(e)}"
        )


@app.post('/verify_signature')
def verify_signature(
    data: str, signature: str, source: str = 'unknown'
):
    is_valid = hmac.compare_digest(
        sign(data, MASTER_KEY), signature
    )
...
def history(limit: int = Query(100, ge=1, le=1000)):
    ...
    try:
        logs = (
            session.query(CryptoLogDB)
            .order_by(CryptoLogDB.created_at.desc())
            .limit(limit).all()
        )
        return [{
            'id': l.id,
