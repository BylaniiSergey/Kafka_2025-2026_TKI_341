...
app = FastAPI(
    title="Neural verify — lower limbs", version="1.1"
)
...
def session_init(body: SessionInit):
    ...
    _last = None
    logger.info(
        "Session patient registered for lower neural verify"
    )
    return {
        "ok": True,
        "patient_id": _registered_patient_id
    }
...
def verify(body: VerifyRequest):
    ...
    if posture == "sitting" and intent in (
        "step_forward", "step_back",
        "walk", "move_forward", "move_backward"
    ):
        return {
            "allowed": False,
