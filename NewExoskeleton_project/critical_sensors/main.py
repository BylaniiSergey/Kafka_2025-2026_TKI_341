...
DATABASE_URL = os.getenv(
    'DATABASE_URL', 'sqlite:///critical_sensors.db'
)
...
def read_critical_sensors() -> dict:
    return {
        'joint_angle': 45.0 + random.gauss(0, 0.2),
        'joint_angular_velocity': random.gauss(0, 0.5),
    ...
    while True:
        ...
        try:
            session.add(CriticalReadingDB(
                **{
                    k: v for k, v in _last_reading.items()
                    if k != 'timestamp'
                }
            ))
            session.commit()
...
app = FastAPI(title="Critical Sensors", version="2.1")
