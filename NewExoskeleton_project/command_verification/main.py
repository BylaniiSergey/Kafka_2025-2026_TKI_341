...
def generate_token(command: str, source: str) -> str:
    ...
    return hashlib.sha256((payload + os.getenv('SECRET_SALT', 'salt')).encode()).hexdigest()[:16]

def save_log(command: str, source: str, passed: bool, reason: str):
    session = SessionLocal()
    try:
        session.add(VerificationLogDB(command=command, source=source, passed=passed, reason=reason))
        session.commit()
    finally: session.close()

app = FastAPI(title="Command Verification", version="2.0")
...
def verify(body: VerifyRequest):
    ...
    expires = datetime.now(timezone.utc).timestamp() + 300  # 5 минут
    save_log(body.command, body.source, True, "verified")
    logger.info(f"Command verified: {body.command} from {body.source}")
    
    return VerifyResponse(trusted=True, token=token, reason="verified", expires_at=datetime.fromtimestamp(expires, tz=timezone.utc).isoformat())
...
def history(limit: int = 100):
    ...
    try:
        ...
        return [{'id': l.id, 'command': l.command, 'source': l.source, 'passed': l.passed, 'reason': l.reason, 'created_at': l.created_at.isoformat()} for l in logs]
    finally: session.close()
