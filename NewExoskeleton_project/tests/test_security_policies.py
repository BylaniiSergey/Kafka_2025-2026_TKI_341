...
def _find_security_monitor_dir() -> Path:
    """
    Найти папку security_monitor. Поддерживает два варианта структуры:
      A) корень репозитория содержит NewExoskeleton_project/security_monitor/
      B) тесты запускаются прямо из NewExoskeleton_project/, и тогда
         security_monitor/ лежит рядом с tests/
    """
    candidates = (
        _ROOT / "NewExoskeleton_project" / "security_monitor",
        _ROOT / "security_monitor",
    )
    ...
    raise FileNotFoundError(
        f"Папка security_monitor не найдена. Проверены пути:\n  {tried}"
    )


def _load_security_module(name: str):
    """
    Загружаем security_monitor/<name>.py как модуль security_monitor.<name>,
    чтобы относительные импорты внутри (например, from .policies import ...)
    работали.
    """
    pkg_path = _find_security_monitor_dir()
